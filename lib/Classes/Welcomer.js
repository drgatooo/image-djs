...
module.exports = class Welcomer {
	...
	async _setup(options) {
		...
		if ('font1' in options) {
			if (typeof options.font1 != 'string')
				throw new Error(
					'El "font2" debe ser un string. Debes importar la fuente previamente con canvas (npm i canvas)'
				);

			this.font = options.font1;
		} else
		...
		if ('font2' in options) {
			...
			this.font = options.font2;
		} else
			this.font = 'bold 32px "DejaVu Sans Condensed", "Arial Unicode MS", segoe-ui-emoji, Sans';

		if ('avatarBorderColor' in options) {
			if (typeof options.borderColor != 'string')
				throw new TypeError('El "avatarBorderColor" debe ser un string');

			if (/^#[0-9a-f]{6}$/i.test(options.borderColor))
				throw new Error('El "avatarBorderColor" debe ser un color HEX (por ejemplo: "#acb221")');

			this.avatarBorderColor = options.avatarBorderColor;
		} else this.avatarBorderColor = '#ffffff';
	}
	 ...
	 */
	async build(toAttachment = true, attachmentName = 'welcomecard-drgato.png') {
		...
		const defFont1 = 'bold 52px "DejaVu Sans Condensed", "Arial Unicode MS", segoe-ui-emoji, Sans';
		const defFont2 = 'bold 32px "DejaVu Sans Condensed", "Arial Unicode MS", segoe-ui-emoji, Sans';

		ctx.font = this.font1 || defFont1;
		ctx.fillStyle = 'white';
		ctx.fillText(`${this.username}`, 375, 300, 500);
		ctx.font = this.font2 || defFont2;
		ctx.fillText(this.subtitle || '¡Bienvenido al servidor!', 375, 345, 500);
