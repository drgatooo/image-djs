...
module.exports = class RankCard {
	 ...
	 */
	setXP(xp) {
		if (!xp) throw new Error('Falta el parametro "xp"');
	...
	setRequiredXP(requiredXp) {
		if (!requiredXp) throw new Error('Falta el parametro "requiredXp"');
	 ...
	 */
	setLevel(level) {
		if (!level) throw new Error('Falta el parametro "level"');
	...
	async buildImage(toMessageAttachment, attachmentName = 'level.png') {
		...
		if (!this.level) throw new Error('Falta configurar el level');
		if (!this.rank) throw new Error('Falta configurar el rank');
		if (!this.requiredXP) throw new Error('Falta configurar el requiredXP');
		if (!this.username) throw new Error('Falta configurar el username');
		if (!this.xp) throw new Error('Falta configurar el xp');
